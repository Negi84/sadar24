...
class CheckoutController extends Controller
{
         ...
         if(auth()->user() != null) {
            ...
            if ($carts && count($carts) > 0) {
                foreach ($carts as $key => $cartItem) {
					...
					$tax += ($cartItem['shipping_cost'] * 0.18) * $cartItem['quantity'];
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            $status = 1;
            // return view('frontend.payment_select', compact('carts', 'shipping_info', 'total'));
            return compact('carts', 'shipping_info', 'total','status');
        ...
        if ($coupon != null) {
            if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date) {
               // if (CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null) {
                    ...
                    if ($coupon->type == 'cart_base') {
                        ...
                        foreach ($carts as $key => $cartItem) {
                            foreach ($coupon_details as $key => $coupon_detail) {
								...
								$product = \App\Product::select('category_id')->where('id', $cartItem['product_id'])->first();
								$category_id = $product->category_id;
								if ($coupon_detail->category_id == $category_id) {
                                    if ($coupon->discount_type == 'percent') {
                                        $coupon_discount += ($cartItem['price'] * $coupon->discount / 100) * $cartItem['quantity'];
                                    } elseif ($coupon->discount_type == 'amount') {
                                        $coupon_discount += $coupon->discount * $cartItem['quantity'];
                    ...
                    Cart::where('user_id', Auth::user()->id)->update(['discount' => $coupon_discount / count($carts),'coupon_code' => $request->code,'coupon_applied' => 1]);
    ...
    {
        Cart::where('user_id', Auth::user()->id)->update(['discount' => 0.00,'coupon_code' => '','coupon_applied' => 0]);
        $coupon = Coupon::where('code', $request->code)->first();
        $carts = Cart::where('user_id', Auth::user()->id)->get();
        $shipping_info = Address::where('id', $carts[0]['address_id'])->first();
    ...
    public function order_confirmed()
    {
        ...
        $order_detail = Cart::where('user_id',$combined_order->user_id)->get();
        ...
        foreach($order_detail as $index => $item){
            ...
            $feed_contents[] =[
                'item_id'=> $item->id,
                'item_name'=> Product::where('id',$item->product_id)->pluck('name')->first(),
                ...
                'index'=> $index,
                'item_brand'=> "Sadar24",
                'item_category'=> category::where('id',Product::where('id',$item->product_id)->pluck('category_id')->first())->pluck('name')->first(),
                'item_category2'=> category::where('id',Product::where('id',$item->product_id)->pluck('category_id')->first())->pluck('name')->first(),
                'item_category3'=> category::where('id',Product::where('id',$item->product_id)->pluck('category_id')->first())->pluck('name')->first(),
                'item_category4'=> category::where('id',Product::where('id',$item->product_id)->pluck('category_id')->first())->pluck('name')->first(),
                'item_category5'=> category::where('id',Product::where('id',$item->product_id)->pluck('category_id')->first())->pluck('name')->first(),
                'item_list_id'=> $item->id,
                'item_list_name'=> Product::where('id',$item->product_id)->pluck('name')->first(),
        ...
        return view('frontend.order_confirmed', compact('combined_order','contents','feed_contents'));
    }
}
