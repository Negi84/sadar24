...
class CartController extends Controller
{
    ...
    public function cartCount(){
        $carts = Cart::where('user_id', auth()->user()->id)->get();
        $arr = array('status'=>true, 'cart_count'=>count($carts));
        return response()->json($arr);
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $str = '';
        $tax = 0;
        // dd((int)$request->quantity, $product->min_qty,$product->digital);
        if($product->auction_product == 0){
            // if($product->digital != 1 && (int)$request->quantity < $product->min_qty) {

            //     $arr = array('status'=>false, 'msg'=>'Product Is Out Of Stock!');
            //     return response()->json($arr);
            ...
            // dd($product->variant_product,$str);
            // $data['color'] = $str;
            // $str = $request->variant;
            $data['variation'] = $str;
            ...
            if ($discount_applicable) {
                if($product->discount_type == 'percent'){
                    $price -= round(($price*$product->discount)/100);
                }
                elseif($product->discount_type == 'amount'){
                    $price -= $product->discount;
                }
                ...
                foreach ($carts as $key => $cartItem){
                    ...
                    if($cartItem['product_id'] == $request->id) {
                        ...
                        if(($str != null && $cartItem['variation'] == $str) || $str == null){
                            ...
                            $wishlistProduct = Wishlist::where(['product_id' => $product->id, 'user_id' => $user_id])->count();
                            if ($wishlistProduct == 0) {
                            } else {
                                Wishlist::where(['product_id' => $product->id, 'user_id' => $user_id])->delete();
                            }
                            $cartItem->save();
                        }
                    }
    ...
    public function reOrder(Request $request,$id)
    {
        ...
        // dd($id);
        $order_query = OrderDetail::where('order_id', $id)->get();

        foreach($order_query as $order){
        $product = Product::find($order->product_id);
        // return $product->stocks->where('variant', $order->variation)->first();
    ...
    public function changeQuantity(Request $request)
    {
        ...
        if ($cart != null) {
            ...
            // dd($cart->product->stocks->where('variant', $cart->variation)->first()->qty);
