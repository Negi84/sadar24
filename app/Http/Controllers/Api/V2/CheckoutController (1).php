...
class CheckoutController
{
    ...
    public function checkout(Request $request){
        ...
        $arr = array('status'=>true, 'carts'=>$carts, 'total'=>$total);
        return response()->json($arr);
    ...
    public function order_place(Request $request){
        ...
        foreach ($seller_products as $seller_product) {
            ...
            foreach ($seller_product as $cartItem) {
                ...
                $product_variation = $cartItem['product_id'];

                $product_stock = $product->stocks->where('product_id', $product_variation)->first();
                if ($product->digital != 1 && $cartItem['quantity'] > $product_stock->qty) {

                    $msg = translate('The requested quantity is not available for ') . $product->getTranslation('name');
        ...
        if ($combined_order_id != null) {
            ...
            if ($request->payment_option == 'razorpay') {
                ...
                $arr = array('status'=>true, 'msg'=>"Your order has been placed successfully", 'combined_order_id'=>$combined_order_id);
                return response()->json($arr);
                ...
                if ($user->balance >= $combined_order->grand_total) {
                    ...
                    $arr = array('status'=>true, 'msg'=>'Order placed succesfully', 'combined_order_id'=> $combined_order_id);
                    return response()->json($arr);
