...
class BusinessSettingsController extends Controller
{
    ...
    public function vendor_commission_update(Request $request){
	 ...
	 public function sla_charge(Request $request)
    {
        return view('backend.sellers.seller_sla_setting.index');
    }

    public function sla_charge_update(Request $request){
        foreach ($request->types as $key => $type) {
            $business_settings = BusinessSetting::where('type', $type)->first();
            if($business_settings!=null){
                $business_settings->value = $request[$type];
        ...
        flash(translate('Seller SLA Charges updated successfully'))->success();
        return back();
    }
