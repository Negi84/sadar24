...
@section('panel_content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <a href="{{ static_asset('download/product_bulk_demo.csv') }}" download><button class="btn btn-primary mt-2">{{ translate('Download CSV') }}</button></a>
        </div>
    </div>
        ...
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('bulk_product_upload') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-0 text-right">
                    <button type="submit" id="hid" class="btn btn-primary" onclick="hidfunction()">{{translate('Upload CSV')}}</button>
                </div>
            </form>
        </div>
    </div>
