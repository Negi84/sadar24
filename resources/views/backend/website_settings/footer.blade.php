...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <i class="las la-times"></i>
            ...
            </div>' data-target=".w3-links-target_1">
                                        {{ translate('Add New') }}
                                    </button>
                                </div>
                                <div class="text-right">
                ...
                <div class="col-lg-12">
                    <div class="card shadow-none bg-light border-dark" id="footerLinkRow">
                        <div class="card-header">
                            <h6 class="mb-0">{{ translate('Keywords Link') }}</h6>
							<button type="submit" onclick="addFooterLinkRow(this)" class="btn btn-primary">{{ translate('Add new') }}</button>
                        </div>
                        @php
                            $i = 0;
                            $status = 'true';
                        @endphp
                        @while ($status == 'true')
                            @if (get_setting('widget_' . $i, null, $lang) != null)
                                <div class="card-body">
                                    <form action="{{ route('business_settings.update') }}" method="POST"
                                        enctype="multipart/form-data">
                                        @csrf
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" placeholder="Widget title"
                                                name="widget_{{ $i }}"
                                                value="{{ get_setting('widget_' . $i, null, $lang) }}">
                                        </div>
                                        <div class="form-group">
                                            <label>{{ translate('Links') }} - ({{ translate('Translatable') }}
                                                {{ translate('Label') }})</label>
                                            <div class="w3-links-target-{{$i}}">
                                                <input type="hidden" name="types[][{{ $lang }}]"
                                                    value="widget_{{ $i }}_labels">
                                                <input type="hidden" name="types[]"
                                                    value="widget_{{ $i }}_links">
                                                @if (get_setting('widget_' . $i . '_labels', null, $lang) != null)
                                                    @foreach (json_decode(get_setting('widget_' . $i . '_labels', null, $lang), true) as $key => $value)
                                                        <div class="row gutters-5">
                                                            <div class="col-4">
                                                                <div class="form-group">
                                                                    <input type="text" class="form-control"
                                                                        placeholder="{{ translate('Label') }}"
                                                                        name="widget_{{ $i }}_labels[]"
                                                                        value="{{ $value }}">
                                                                </div>
                                                            </div>
                                                            <div class="col">
                                                                <div class="form-group">
                                                                    <input type="text" class="form-control"
                                                                        placeholder="http://"
                                                                        name="widget_{{ $i }}_links[]"
                                                                        value="{{ json_decode(get_setting('widget_' . $i . '_links'), true)[$key] }}">
                                                                </div>
                                                            </div>
                                                            <div class="col-auto">
                                                                <button type="button"
                                            ...
                                            <button type="button" class="btn btn-soft-secondary btn-sm"
												...
												<div class="col-4">
												<div class="form-group">
												<input type="text" class="form-control" placeholder="{{ translate('Label') }}" name="widget_{{ $i }}_labels[]">
												</div>
												</div>
												<div class="col">
												<div class="form-group">
												<input type="text" class="form-control" placeholder="http://" name="widget_{{ $i }}_links[]">
												</div>
												</div>
												<div class="col-auto">
												<button type="button" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger" data-toggle="remove-parent" data-parent=".row">
                        ...
                        {{-- <div class="card-body">
                            <form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
    							...
    							<div class="form-group">
    								...
    								<input type="text" class="form-control" placeholder="Widget title" name="widget_two" value="{{ get_setting('widget_two',null,$lang) }}">
    							</div>
    			                <div class="form-group">
    								<label>{{ translate('Links') }} - ({{ translate('Translatable') }} {{ translate('Label') }})</label>
    								<div class="w3-links-target">
    									<input type="hidden" name="types[][{{ $lang }}]" value="widget_two_labels">
    									<input type="hidden" name="types[]" value="widget_two_links">
    									@if (get_setting('widget_two_labels', null, $lang) != null)
...
<script>
	function addFooterLinkRow(e){
		// console.log($("#footerLinkRow .card-body").children().length);
		var num = $("#footerLinkRow .card-body").children().length;
		var linkRow = `
		<div class="card-body">
                                        <form action="{{ route('business_settings.update') }}" method="POST"
                                            enctype="multipart/form-data">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" placeholder="Widget title"
                                                    name="widget_`+num+`"
                                                    value="{{ get_setting('widget_`+num+`', null, $lang) }}">
                                            </div>
                                            <div class="form-group">
                                                <label>{{ translate('Links') }} - ({{ translate('Translatable') }}
                                                    {{ translate('Label') }})</label>
                                                <div class="w3-links-target-`+num+`">
                                                    <input type="hidden" name="types[][{{ $lang }}]"
                                                        value="widget_`+num+`_labels">
                                                    <input type="hidden" name="types[]"
                                                        value="widget_`+num+`_links">
                                                    @if (get_setting('widget_`+num+`' . '_labels', null, $lang) != null)
                                                        @foreach (json_decode(get_setting('widget_`+num+`' . '_labels', null, $lang), true) as $key => $value)
                                                            <div class="row gutters-5">
                                                                <div class="col-4">
                                                                    <div class="form-group">
                                                                        <input type="text" class="form-control"
                                                                            placeholder="{{ translate('Label') }}"
                                                                            name="widget_`+num+`_labels[]"
                                                                            value="{{ $value }}">
                                                                    </div>
                                                                </div>
                                                                <div class="col">
                                                                    <div class="form-group">
                                                                        <input type="text" class="form-control"
                                                                            placeholder="http://"
                                                                            name="widget_`+num+`_links[]"
                                                                            value="{{ json_decode(get_setting('widget_`+num+`' . '_links'), true)[$key] }}">
                                                                    </div>
                                                                </div>
                                                                <div class="col-auto">
                                                                    <button type="button"
										...
										<div class="col-4">
										<div class="form-group">
										<input type="text" class="form-control" placeholder="{{ translate('Label') }}" name="widget_`+num+`_labels[]">
										</div>
										</div>
										<div class="col">
										<div class="form-group">
										<input type="text" class="form-control" placeholder="http://" name="widget_`+num+`_links[]">
										</div>
										</div>
										<div class="col-auto">
										<button type="button" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger" data-toggle="remove-parent" data-parent=".row">
