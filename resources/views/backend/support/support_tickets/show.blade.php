    ...
    <div class="card">
        <div class="card-header row gutters-5">
            <div class="text-center text-md-left">
                <h5 class="mb-md-0 h5">{{ $ticket->subject }} #{{ $ticket->code }} 
				<span class="badge badge-inline badge-secondary ml-2 text-capitalize"> 
				   {{ translate($ticket->status) }} 
				</span>
				</h5>
               <div class="mt-2">
					<span class="badge badge-inline badge-danger ml-2 text-capitalize"> 
                       {{ $ticket->user->user_type }}
					</span>
					<span class="ml-2"> {{ $ticket->user->name }}</span>
					<p class="mt-2"><strong>Order ID : </strong>{{ $ticket->order_id }} <br>
					<strong>Date : </strong>{{ $ticket->created_at }} </p>	
					
               </div>
            </div>
        </div>
        <div class="card-body">
			@if(ucfirst($ticket->status) != 'Solved')
            <form action="{{ route('support_ticket.admin_store') }}" method="post" id="ticket-reply-form" enctype="multipart/form-data">
				...
				<input type="hidden" name="created_at" value="{{ $ticket->created_at }}" required>
