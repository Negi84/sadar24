                ...
                @if ($type == 'All')
                    <div class="col-md-2 ml-auto">
                        <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id"
                            name="user_id" data-live-search="true" onchange="sort_products()">
                            <option value="">{{ translate('All Sellers') }}</option>
                            @foreach (App\User::where('user_type', '=', 'admin')->orWhere('user_type', '=', 'seller')->get()
            as $key => $seller)
                ...
                <div class="col-md-2 ml-auto">
                    <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="type" id="type"
                        ...
                        <option value="published,desc"
                            @isset($col_name, $query) @if ($col_name == 'published' && $query == 'desc') selected @endif @endisset>
                            {{ translate('Published') }}</option>
                        <option value="published,asc"
                            @isset($col_name, $query) @if ($col_name == 'published' && $query == 'asc') selected @endif @endisset>
                            {{ translate('Un Published') }}</option>
                    </select>
                </div>
                <div class="col-md-2">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="row gutters-5 w-200px w-md-300px mw-100">
                                        <div class="col-auto">
                                            <img src="@if (count($photos) > 1) @if ($photos[0] == 0){{ uploaded_asset($photos[1]) }}@else{{ uploaded_asset($photos[0]) }} @endif
                            @else
                                {{ uploaded_asset($photos[0]) }} @endif" alt="Image" class="size-50px img-fit">
                                        </div>
                                        <div class="col">
                                            <span
                                ...
                                <td>
                                    ...
                                    {{ translate('times') }} </br>
                                    <strong>{{ translate('MRP') }}:</strong>
                                    {{ single_price($product->unit_price) }} </br>
                                    <strong>{{ translate('Selling Price') }}:</strong>
                                    {{ single_price($product->unit_price - $product->discount) }} </br>
    ...
    echo 'checked';
                                ...
                                <td class="text-right">
                                    ...
                                    <a class="btn btn-soft-warning btn-icon btn-circle btn-sm"
                                        onclick="dubProd('{{ route('products.duplicate', ['id' => $product->id, 'type' => 'In House']) }}')"
